...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    setOrderError("");
    if (!isAuthenticated()) {
      setOrderError(t("cart.loginToOrder"));
    ...
    try {
      const orderId = await placeOrder(items, cartTotal, shipping);
      clearCart();
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24">
              ...
              {orderError && (
                <p className="text-red-600 text-sm mb-3">{orderError}</p>
              )}
              <button
                type="button"
                onClick={handlePlaceOrder}
                disabled={placingOrder}
                className="w-full btn-gold text-white font-semibold py-3 rounded-lg text-sm cursor-pointer disabled:opacity-60"
              >
                {placingOrder ? t("cart.placingOrder") : t("cart.placeOrder")}
              </button>
              <Link
                to="/catalog"
